...
4. The HTTP proxy (`src/proxy/httpProxy.js`, with header/path-rewrite helpers in `requestUtils.js` and mock compiling/matching/serving split into `mockMatcher.js`/`mockResponder.js`) receives the request:
...
- **8282 by default, user-configurable** — HTTP proxy server (`settings.proxyPort`, 1-65535, clamped/sanitized in `setSettings()`; rejects a value equal to the PAC server's port and keeps the previous value instead). Fixed across restarts so browsers reading the PAC file and CLI tools that don't honour the OS auto-proxy setting (`curl`, scripts, API clients) can both rely on it — e.g. `curl -x http://127.0.0.1:8282 http://myapp.local` or `export http_proxy=http://127.0.0.1:8282`. A port change only takes effect the next time the proxy is started (`HttpProxy._startHttpServer(port)`); tests that don't pass `settings.proxyPort` still get the OS-assigned dynamic port (`0`) so parallel test runs don't collide on a fixed port.
...
`src/renderer/components/` is organized into subdirectories by role: `layout/` (`Sidebar.jsx`, `Titlebar.jsx`), `modals/` (`Modal.jsx` — Escape-to-close plus backdrop-click-to-close, `ConfirmModal.jsx` — generic confirm/cancel dialog used for destructive actions in place of native `confirm()`, `MappingModal.jsx`, `MockModal.jsx`, `MockRegexHelpPane.jsx`, `AboutModal.jsx`, `ExportModal.jsx`, `AppModals.jsx` — renders the set of modals owned by `App.jsx`), `settings/` (`ProxySection.jsx`, `CertificateSection.jsx`, `RequestLogsSection.jsx`, `HealthChecksSection.jsx`, `PreferencesSection.jsx`, `CliAccessSection.jsx` — one-time setup instructions (proxy env vars, CA trust) for curl/scripts/API clients that don't honour the OS auto-proxy setting — extracted sections rendered by `SettingsView.jsx`), and `utilities/` (`Toast.jsx`, `Tooltip.jsx`). View-level components (`App.jsx`, `DashboardView.jsx`, `MappingsView.jsx`, `MocksView.jsx`, `LogView.jsx`, `LogDetailPanel.jsx`, `SettingsView.jsx`, `ConditionListEditor.jsx`, `HeaderListEditor.jsx`) live directly under `components/`.

`eslint.config.mjs` intentionally ignores `src/renderer/scripts.js` (the esbuild output). The ESLint config covers `.js` files and `.jsx` files — the `.jsx` block extends `@eslint-react/eslint-plugin`'s `recommended` preset (flat rule IDs, e.g. `@eslint-react/dom-no-unknown-property`; `jsx-uses-vars` is unnecessary since ESLint 10 tracks JSX references natively) plus `eslint-plugin-react-hooks` (`rules-of-hooks`, `exhaustive-deps`).
...
`CertManager` (`src/proxy/certManager.js`) is a **singleton** via `static instance`. On first run, `ensureCA()` (async, pre-warmed at startup in `main.js`) generates a 2048-bit RSA root CA (`ca.crt` + `ca.key`) and persists it to `app.getPath('userData')/certs/`; the constructor only loads an existing CA from disk and never generates. All RSA keygen goes through node:crypto's async `generateKeyPair` (native, runs on the libuv threadpool) and is converted to node-forge key objects for cert signing — node-forge's own pure-JS keygen would block the main process for seconds. Per-domain leaf certs are generated lazily on first HTTPS CONNECT for that hostname via the async `getCert()` (concurrent requests for the same hostname share one in-flight promise), cached in memory and on disk; private key files are written with mode `0o600`. The `SNICallback` on the internal HTTPS server selects the correct cert per connection. `getCAExpiry()` returns `null` when no CA is loaded (e.g. after `ssl:delete-ca`); the `ssl:get-ca-expiry` handler awaits `ensureCA()` first. If `certDir` ever needs to change, set `CertManager.instance = null` before calling `getInstance()` again.
...
`pathRewriteFrom`/`pathRewriteTo` (both default `''`, sanitized via the shared `sanitizePathRewrite()` — trimmed, given a leading `/` if non-empty, trailing `/` stripped unless the value is just `/`) let a mapping strip or replace a path prefix before forwarding to the backend. `rewritePath(mapping, pathWithQuery)` (`src/proxy/requestUtils.js`, called directly by `HttpProxy`) rewrites the path only when `pathRewriteFrom` is non-empty and the request pathname is exactly `pathRewriteFrom` or starts with `pathRewriteFrom + '/'`, replacing the matched prefix with `pathRewriteTo` (falling back to `/` if the result is empty) while preserving the query string; it is called from `_handleRequest`, `_handleDecryptedRequest`, and `_handleWebSocketUpgrade` (applied to `req.url` when replaying the upgrade request line) right before the backend request/connection is built. Rewriting happens *after* the mock lookup, so mock rules always match the original, pre-rewrite request path — rewriting only affects what is sent to the real backend. Raw HTTPS CONNECT tunnels (`_tunnelRaw`, used when global HTTPS is disabled) are opaque TCP and are not affected by path rewriting.

`requestHeaders`/`responseHeaders` are arrays of `{ name, value }` pairs (sanitized via `sanitizeHeaders()`, default `[]`), editable per-mapping in `MappingModal.jsx`. `applyHeaderOverrides()` (`src/proxy/requestUtils.js`, called directly by `HttpProxy`) lowercases each `name` and sets it on the headers object, so overrides replace rather than duplicate existing headers — `requestHeaders` are applied to the outgoing backend request (including WebSocket upgrade replays) and `responseHeaders` to the response written back to the client.

`windowBounds` (`{ width, height }`, default `940x680`) is read via `store.getWindowBounds()` when creating the `BrowserWindow` and saved via `store.setWindowBounds()` on `resize` (debounced 500 ms) and on the window's `close` event, so the app reopens at its last size even after a crash or force-quit.

Settings defaults live in the exported `DEFAULT_SETTINGS` object in `store.js`: `{ httpsEnabled: true, startOnLaunch: true, proxyPort: DEFAULT_PROXY_PORT, colorMode: 'auto', locale: 'en', iconMode: 'both', dashboardEnabled: false, logMaxEntries: DEFAULT_LOG_MAX_ENTRIES, loggingEnabled: true, logHeadersEnabled: false, logBodyEnabled: false, healthCheckEnabled: false, healthCheckIntervalMs: DEFAULT_INTERVAL_MS, healthCheckTimeoutMs: DEFAULT_TIMEOUT_MS }`. `getSettings()` merges `DEFAULT_SETTINGS` under the stored object, so settings keys added in later versions resolve to their default for existing stores (the electron-store schema default only applies when the whole `settings` key is missing) — renderer code can rely on every key being present and doesn't need per-key fallbacks. `logMaxEntries`'s default is imported from `requestLog.js`'s `DEFAULT_MAX_ENTRIES`. `healthCheckIntervalMs` (1 min default, clamped 5s-300s) and `healthCheckTimeoutMs` (2s default, clamped 500ms-30s) come from `src/proxy/healthChecker.js`'s exported defaults and are clamped in `setSettings()`. `proxyPort` (`DEFAULT_PROXY_PORT = 8282`, clamped 1-65535 in `setSettings()`) is the fixed port `HttpProxy` listens on — see [Ports](#ports). `iconMode` is validated to one of `both`, `tray`, `dock` (default `both`) and `colorMode` to `auto`/`light`/`dark` (default `auto`). The electron-store schema also carries per-property constraints (settings enums/ranges, mapping `port` 1-65535, mock `statusCode`/`delayMs` bounds) as a second enforcement layer behind the sanitizers — setSettings()/addMapping()/addMock() always write valid values, and the schema guards against e.g. a hand-edited config.json.
...
Mock compiling and matching lives in `src/proxy/mockMatcher.js`, not `httpProxy.js`: `compileConditions()` pre-lowercases `header` condition keys and pre-compiles `regex` operator values (a compile failure skips the whole mock, same as an invalid `pathPattern`); `findMock()` additionally calls `conditionsMatch()`/`conditionMatches()` against an `extra` object (`{ headers, query, body }`) built from the request. `HttpProxy.updateMocks()` delegates to `compileMockRules()`, and `HttpProxy._continueRequest()` calls `findMock()` directly. Because a `body` condition needs the full request body before a match decision can be made, `compileMockRules()` also returns `needsBody` (a `Set<mappingId>`, stored as `HttpProxy.mocksNeedBody`); `_dispatch()` checks this set and, when present, buffers the request body via `_readBodyForMatching()` before calling `_continueRequest()` — otherwise requests stream through unbuffered as before. Buffering is capped at `MAX_MOCK_BODY_MATCH_BYTES` (1 MB): a larger body is treated as "no body" for matching (body conditions fail), the buffered part is replayed to the backend, and the paused remainder of the stream is piped after it. When the body was fully pre-buffered, `_continueRequest()` replays it (`proxyReq.end(...)` / passing it into `serveMock()`) instead of re-reading `req`, since a request stream can only be consumed once.
...
- **Time-series charts** — requests per minute, error rate (%), and average latency (ms) over the last 30 minutes, rendered with chart.js / react-chartjs-2. Each chart shows a "no data yet" placeholder instead of a flat-zero line when there are no log entries.

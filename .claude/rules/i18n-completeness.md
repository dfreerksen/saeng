...
This project supports 15 locales (`src/i18n/locales/*.json`), with `en.json` as the canonical source of truth for keys.

When adding, renaming, or removing a user-facing string key:

- Add/update the key in `en.json` AND every other locale file (`ar`, `be`, `bg`, `de`, `es`, `fr`, `ja`, `ko`, `pt`, `ru`, `th`, `uk`, `vi`, `zh`), in the same position as `en.json` so diffs stay readable.
